...
class BoardRepository extends Repository
{
    ...
    function updateThreadCount($board_id, $threadCountChange)
    {
        $stmt = $this->connection->prepare("UPDATE boards SET total_threads = total_threads + :total_threads WHERE board_id = :board_id");
        $stmt->bindParam(':board_id', $board_id);
        $stmt->bindParam(':total_threads', $threadCountChange);
        $stmt->execute();
    }
}